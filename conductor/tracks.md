...
## 🌱 Публичные и системные доски

- [x] **Трек: Публичные комнаты + Системные доски**
  *Приоритет: 🟡 Средний* *(завершён ✅ 2026-05-07)*
  *Описание: Флаг `is_public` и `is_system` на Room. Эндпоинты `/rooms/public` и `/rooms/system`. Сид 6 системных комнат (`_help`, `_about`, `_me`, `_log`, `_team`, `_terms`). Главная страница: раздел «Открытые доски» + «Системные доски». Упрощённое создание комнат: приватная по умолчанию, в Advanced — переключатель public/private.*
