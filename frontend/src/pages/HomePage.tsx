...
import { ArrowRight, Clipboard, Smartphone, Monitor, Zap, Globe, Sun, Moon, Hash, LogIn } from 'lucide-react';
...
// Validate room name: 2-32 chars, alphanumeric + _ and -
...
export default function HomePage() {
  ...
  // After login: check for a pending room saved before OAuth redirect
  useEffect(() => {
    if (isLoading || didAutoRedirect.current) return;

    const pendingRoom = localStorage.getItem(PENDING_ROOM_KEY);
    if (pendingRoom && user) {
      // User just came back from OAuth — go to the room they typed
  ...
  // Auto-redirect to the single room if user has exactly one room
  useEffect(() => {
    if (isLoading || roomsLoading || didAutoRedirect.current) return;
    if (!user || myRooms.length !== 1 || roomInput.trim()) return;
    // Only auto-redirect if there's no pending room (handled above)
  ...
  // Before OAuth redirect: save the typed room name so we can restore it
      ...
      <main className="max-w-4xl mx-auto px-6 sm:px-10 pt-10 sm:pt-16 pb-16">
        ...
        <div className="text-center mb-10">
          ...
          <div className="max-w-lg mx-auto">
            <div className={`flex items-center gap-2 bg-white dark:bg-slate-800 rounded-2xl border-2 transition-all shadow-lg ${
              inputError
                ? 'border-red-400 dark:border-red-500'
                : 'border-slate-200 dark:border-slate-700 focus-within:border-indigo-400 dark:focus-within:border-indigo-500 focus-within:shadow-indigo-100 dark:focus-within:shadow-indigo-900/30'
            }`}>
              ...
              <input
                ...
                className="flex-1 py-4 bg-transparent text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 outline-none text-base font-medium"
                maxLength={32}
                autoComplete="off"
                autoCorrect="off"
                spellCheck={false}
            ...
            {inputError && (
              <p className="mt-2 text-sm text-red-500 dark:text-red-400 text-left px-1">{inputError}</p>
            )}

            <p className="mt-2 text-xs text-slate-400 dark:text-slate-500 text-center">
              {lang === 'RU'
                ? 'Введи имя вроде «Silenthill» или оставь пустым — сгенерируем случайное'
                : 'Type a name like "Silenthill" or leave empty for a random one'}
            </p>
            ...
            {!isLoading && !user && (
              <button
                ...
                className="mt-4 w-full py-3 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-medium rounded-xl border border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-500 transition-all hover:shadow-md flex items-center justify-center gap-2"
              >
                <LogIn size={16} />
                {t('login') || (lang === 'RU' ? 'Войти, чтобы сохранять комнаты' : 'Sign in to save rooms')}
              </button>
            )}
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {myRooms.map(room => (
                <button
                  ...
                  className="text-left p-4 bg-white/80 dark:bg-slate-800/60 backdrop-blur-sm rounded-2xl border border-slate-200/60 dark:border-slate-700/60 hover:border-indigo-300 dark:hover:border-indigo-500 hover:shadow-lg transition-all group"
                >
                  <div className="font-mono text-sm font-bold text-indigo-600 dark:text-indigo-400 mb-1 group-hover:text-indigo-700 dark:group-hover:text-indigo-300 transition-colors truncate">
                    #{room.id}
                  </div>
                  <div className="text-xs text-slate-400 dark:text-slate-500">
                    TTL: {room.ttl === 'forever' ? '∞' : room.ttl}
                  </div>
                </button>
              ))}
            </div>
        ...
        {/* Loading rooms */}
        {user && roomsLoading && (
          <div className="text-center py-6 text-slate-400 dark:text-slate-500 text-sm animate-pulse">
            {lang === 'RU' ? 'Загружаем ваши комнаты...' : 'Loading your rooms...'}
          </div>
        )}
